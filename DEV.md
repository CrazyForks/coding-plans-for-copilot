...
- Default request headers match native Custom Endpoint: `openai-chat` / `openai-responses` send `Authorization: Bearer`; `anthropic` sends `x-api-key` plus `anthropic-version: 2023-06-01` and does not send `Authorization`.
...
- Thinking effort:
  ...
  - Protocol mapping:
    - `openai-chat`: uses `request.modelOptions.thinkingEffort`, options `none` / `low` / `medium` / `high` / `xhigh` / `max`; the model row `More Actions` default is `high`; `none` sends `thinking: { type: "disabled" }`. The rest send top-level `reasoning_effort` only by default (native Custom Endpoint Chat Completions shape). Uses `request.modelOptions.thinkingType` as the thinking switch; the More Actions default is `default` (omit `thinking`, A11/A12). Explicit `enabled` also sends `thinking: { type: "enabled" }` (A13) for vendors that require that field. Response parts emit thinking before output text (native Custom Endpoint order).
    - `openai-responses`: uses `request.modelOptions.thinkingEffort`, options `low` / `medium` / `high` / `xhigh` / `max`; the model row `More Actions` default is `max`; sends `reasoning: { effort }`. Response parts emit thinking before output text.
    - `anthropic`: uses `request.modelOptions.thinkingType` as the switch; `true` sends `thinking: { type: "adaptive" }`, `false` sends `thinking: { type: "disabled" }`; uses `request.modelOptions.effort` to send `output_config.effort`, options `low` / `medium` / `high` / `xhigh` / `max`. Response parts emit thinking before output text.
